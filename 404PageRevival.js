...
const buttonStyle = "position: relative; min-width: fit-content; height: 80%; background-color: orange; vertical-align: " +
    ...
    "color: white; font-size: 20px; padding: 10px; margin-left: auto; margin-right: 5%; cursor: pointer; overflow: hidden; white-space: nowrap; font-family: 'Segoe UI' !important; text-transform: none !important; font-weight: 500 !important; ";
const textStyle = `color: white; white-space: nowrap; text-overflow: ellipsis; overflow: hidden; margin-right: 1%; margin-bottom: 0; margin-top: 0; font-family: 'Segoe UI' !important; text-transform: none !important; font-size: 20px !important; font-weight: 500 !important;`;
...
(async function() {
    ...
    if (await is404Page(window.location.href)) {
        await infobarExecutor();

    }
    GM_registerMenuCommand('Create Infobar', async function() {
        await infobarExecutor();
    }, 'r');
...
async function infobarExecutor() {
    if (infobarCreated) {
        alert("Infobar has already been created");
        return;
    }
...
async function is404Page(url) {
    try {
        ...
        return errorCodes.includes(status) && txt.search('404') !== -1;
    } catch (err) {
        return false;
    }
